			...
			if (hasXP || userHasCreative) { // only continue if the player has exp or access to creative mode
				...
				if (!((double)f < 0.1)) {
					...
					if (!world.isClient) {
						double range = 36.6d;
						float tickDelta = 1f;
						Vec3d playerEyePos = playerEntity.getEyePos();
						Vec3d lookDir = playerEntity.getRotationVec(tickDelta);
						Vec3d endPos = playerEyePos.add(lookDir.x * range, lookDir.y * range, lookDir.z * range);
						HitResult blockResult = world
							.raycast(new RaycastContext(
								playerEyePos, endPos, RaycastContext.ShapeType.COLLIDER, RaycastContext.FluidHandling.NONE, playerEntity
							));
